...
def concatenateday(dayfiles,kazrfile,pblhfile,windfiles,date_str):
    ...
    resample500 = {}
    intensity500 = intensity[:,16]
    for ct, height in enumerate(height_km[:67].values):
        intensity_hgt = intensity[:,ct]
        resampled_intensity = np.minimum(intensity500,intensity_hgt)
        wprime500 = smoothedV[:,16]
        wprime500 = wprime500.where(resampled_intensity >= 1.008, np.nan)
        resample500[str(height)] = wprime500
    ...
    windspeed_dir = '/thumper/users/scott.powell/code/research-code/BNF/lidar/COR/windspeed_lidar/'
    ...
    updraft_500_resample = Parallel(n_jobs=120)(delayed(define500chords)
                                        (resample500[key], lidar_pbl_height, time_hours, ws[:,16], key, threshold) 
    ...
    updraft_output_dir = '/thumper/users/scott.powell/code/research-code/BNF/lidar/COR/updraft_output/'
    updraft_output_resample_dir = '/thumper/users/scott.powell/code/research-code/BNF/lidar/COR/updraft_output_resample/'
...
def main():
    # Load the CDF file
    fdir = '/thumper/metdata/doppler-lidar/COR/'
    kazrdir = '/thumper/metdata/radar/KAZR_CACTI/'
    pblhdir = '/thumper/metdata/PBLH/CACTI/corceilpblhtM1/'
    # sondedir = '/thumper/metdata/sonde/CACTI/M1/'
    winddir = '/thumper/metdata/doppler-lidar/COR/wind/'
    names = sorted(glob(fdir+"*.cdf"))
    kazrnames = sorted(glob(kazrdir+"*.nc"))
    pblhnames = sorted(glob(pblhdir+"*.cdf"))
