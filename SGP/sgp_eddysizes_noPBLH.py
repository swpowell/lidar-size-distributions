...
def process_stats(time, stats):
    ...
    if time_diff <= 10:
        ...
        filtered_stats = stats.sel(time=nearest_time).where((stats.height >= 100) & (stats.height <= 9999), drop=True)
...
def getwvariance(stats,times):

    # For each pbl_height, find the nearest mean/median w variance between 100 m
    # and the PBL height.

    results = Parallel(n_jobs=100)(delayed(process_stats)(time, stats) for time in times)
...
def definechords(smoothedV,height_km,time_hours,ws,threshold=0.5):
    ...
    def process_eddies_by_height(idh, height, maxz, updraft_mask, time_sec, smoothedV, ws):
        updraft_dicts = []

        if height <= maxz: #min(maxz,lidar_pbl_height.max()):
    ...
    results = Parallel(n_jobs=96)(delayed(process_eddies_by_height)(idh, height, maxz, updraft_mask, time_sec, smoothedV, ws) for idh, height in enumerate(height_km))
...
def define500chords(resample500_level,time_hours,ws500,key,threshold=0.5):
...
def concatenateday(dayfiles,statfile,windfiles,date_str,resamlevel):
    ...
    # wvar = getwvariance(stats,lidar.time)
    ...
    windspeed_dir = '/thumper/users/scott.powell/code-data/research-code/lidar/SGP/windspeed_lidar/'
    ...
    updraft_df = definechords(smoothedV,height_km,time_hours,ws,threshold)
...
def main():
    ...
    for date_str in dates[2800:]: # Still need to do 2100 to 2800 and 2800: for C1.
        ...
        try:
            ...
            concatenateday(dayfiles,statfile,windfiles,date_str,26)
