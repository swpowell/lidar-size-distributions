...
def velocity_running_mean(name, previous_file, next_file, output_dir):
    ...
    if ds_prev is not None and ds_next is not None:
        combined_intensity = xr.concat([ds_prev['intensity'], ds['intensity'], ds_next['intensity']], dim='time')
    elif ds_prev is not None:
        combined_intensity = xr.concat([ds_prev['intensity'], ds['intensity']], dim='time')
    elif ds_next is not None:
        combined_intensity = xr.concat([ds['intensity'], ds_next['intensity']], dim='time')
    else:
        combined_intensity = ds['intensity']
    ...
    for i in range(len(time_values)):
        ...
        # What percentage of values meet intensity threshold?
        A = np.zeros_like(intensity_values)
        A[intensity_values >= 1.008] = 1
        pctgood = A.sum(axis=0)/A.shape[0]
        hgtmask = (pctgood >= 1)
        # Set to NaN if hgtmask not satisfied 
        mean_val = np.mean(radial_velocity_values[mask], axis=0)
        mean_val[hgtmask == False] = np.nan        
...
def process_stats(time, lidar_pbl_height, stats):

    pbl_height = lidar_pbl_height.sel(time=time).values
    
    # Find the nearest time in stats
    nearest_time = stats.time.sel(time=time, method='nearest')
    
    # Check if the nearest time is within 10 minutes
    time_diff = np.abs((nearest_time - time).values / np.timedelta64(1, 'm'))
    if time_diff <= 10:
        stats.load()
        # Filter stats to include only those within 100m to pbl_height at the nearest time
        filtered_stats = stats.sel(time=nearest_time).where((stats.height >= 100) & (stats.height <= pbl_height), drop=True)
        
        if filtered_stats['w_variance'].size > 0:
            # Calculate mean and median
            mean_value = filtered_stats['w_variance'].mean().item()
            median_value = filtered_stats['w_variance'].median().item()
            return (time.values, mean_value, median_value)
        else:
            return (time.values, None, None)
    else:
        return (time.values, None, None)

# ****************************************************************************

def getwvariance(stats, lidar_pbl_height):

    # For each pbl_height, find the nearest mean/median w variance between 100 m
    # and the PBL height.

    results = Parallel(n_jobs=100)(delayed(process_stats)(time, lidar_pbl_height, stats) for time in lidar_pbl_height.time)
...
def definechords(smoothedV,lidar_pbl_height,wvarmean,wvarmed,height_km,time_hours,ws,threshold=0.5):
    ...
    def process_eddies_by_height(idh, height, maxz, lidar_pbl_height, updraft_mask, time_sec, smoothedV, ws):
        updraft_dicts = []

        if height <= min(maxz,lidar_pbl_height.max()):

            # Label the contiguous updraft objects in 1D.
            # NOTE: These will be used as unique IDs to identify updraft objects in the saved output.
            upeddies, num = label(updraft_mask[:,idh])            
                ...
                if dt > 0:
                    ...
                    # Proceed to find its duration and chord length.
                        ...
                        if height*1000 <= lidar_pbl_height[center_time_index]:
                            ...
                            cond = np.isnan(smoothedV[idt.min()-1,idh].values) or np.isnan(smoothedV[idt.max()+1,idh].values) 
        ...
        return updraft_dicts

    # Iterate over each height (range dimension)
    results = Parallel(n_jobs=96)(delayed(process_eddies_by_height)(idh, height, maxz, lidar_pbl_height, updraft_mask, time_sec, smoothedV, ws) for idh, height in enumerate(height_km))
    updraft_df = pd.DataFrame([item for sublist in results for item in sublist])
...
def define500chords(resample500_level,lidar_pbl_height,wvarmean,wvarmed,time_hours,ws500,key,threshold=0.5):
        ...
        for eddy in np.unique(upeddies[upeddies >= 1])[1:]:
            ...
            if dt > 0:
                ...
                # Proceed to find its duration and chord length.
                    ...
                    # Check if the current height is below the lidar_pbl_height at each time
                    # Exclude single time observations.
                    if 795 <= lidar_pbl_height[center_time_index]:
                        ...
                        # A3.append(ws500[center_time_index])
    ...
    updraft_df2 = pd.DataFrame(updraft_dicts)
...
def concatenateday(dayfiles,kazrfile,pblhfile,statfile,windfiles,date_str,resamlevel):

    # from define500 import define500chords
    ...
    t = lidar.time[51000:52800]
    elapsed_time = t - t[0]
    seconds_ahead = elapsed_time / np.timedelta64(1, 's')    
    t = seconds_ahead.values

    fig, ax = plt.subplots(1,1,figsize=(10,6))
    plt.contourf(t[:1000],height_km[:50],X[51000:52000,:50].transpose(),np.arange(-6,6.1,0.5),cmap=cm.get_cmap('RdBu_r'))
    cb = plt.colorbar()
    ax.set_xlabel('Time (s)')
    ax.set_ylabel('Height (km)')
    cb.set_label(r'Vertical velocity (m s$^{-1}$)')
    fig.savefig('sample_w.png')
    ...
    smoothedV = smoothedV[:,height_km<=2]
    intensity = intensity[:,height_km<=2]
    height_km = height_km[height_km<=2]

    # Resample data at 500 m using the SNR from the other heights. 
    # NOTE: This will be a 2D array of a bunch of 1D arrays along 500 m.
    # However, each new 1D array will be NaNed out according to the SNR at other levels.
    resample500 = {}
    intensity500 = intensity[:,resamlevel]


    # Create a mask for resampled_intensity
    mask_resampled_intensity = np.minimum(intensity500, intensity[:, :67]) >= 1.008

    # Create a mask for NaNs in smoothedV
    mask_nan_smoothedV = ~np.isnan(smoothedV[:, :67])

    # Combine masks to get the final mask for wprime500
    final_mask = mask_resampled_intensity & mask_nan_smoothedV[:,resamlevel] & mask_nan_smoothedV 

    # Apply the mask to wprime500
    wprime500 = smoothedV[:, resamlevel].where(final_mask, np.nan)

    # Apply the mask to smoothedV
    smoothedV[:, :67] = smoothedV[:, :67].where(final_mask, np.nan)

    # Update resample500
    for ct, height in enumerate(height_km[:67].values):
        resample500[str(height)] = wprime500[:,ct]


    # # This is a check on the resampling
    # for ct, k in enumerate(resample500):
    #     A = np.isnan(smoothedV[:,ct].values).sum()
    #     B = np.isnan(resample500[k].values).sum()
    #     print(k,A,B)

    # Try this on date_str = dates[2000], or '20190906'
    # BUG: plt.pcolormesh(smoothedV[20300:20600,:].values,vmin=-.5,vmax=.5)
    # BUG: Striped data starts to appear. This is not good! Probably is causing some of
    # the oscillations from height to height in some files in 2021. This could also
    # cause size distribution to skew small. Need a way to exclude this data.
    # NOTE: Problem seems to be when original files have 10 minute mean w removed.
    ...
    # Now get the vertical velocity variance to determine if CBL is active.
    # stats = xr.open_dataset(statfile)
    # wvar = getwvariance(stats,lidar_pbl_height)
    # wvarmean = np.array([i[1] for i in wvar])
    # wvarmed = np.array([i[2] for i in wvar])
    ...
    windspeed_dir = '/thumper/users/scott.powell/code-data/research-code/lidar/SGP/C1_windspeed_lidar/'
    ...
    updraft_output_dir = '/thumper/users/scott.powell/code-data/research-code/lidar/SGP/C1/'
    updraft_output_resample_dir = '/thumper/users/scott.powell/code-data/research-code/lidar/SGP/C1_resample/'
    ...
    #     updraft_df.set_index('Center Datetime')
    #     updraft_500_resample.set_index('Center Datetime')

    # else:
    updraft_df = definechords(smoothedV,lidar_pbl_height,0,0,height_km,time_hours,ws,threshold)
    updraft_500_resample = Parallel(n_jobs=120)(delayed(define500chords)
                                        (resample500[key], lidar_pbl_height, None, None, 
                                        time_hours, ws[:,resamlevel], key, threshold) 
                                        for key in resample500.keys())
    updraft_500_resample = pd.concat(updraft_500_resample, ignore_index=True)
...
def process_lidar_file(i, name, names, output_dir):
    ...
    # BUG: There is a problem with this at edge of data availability, where
    # stripedness appears in data along end data points. Discard a column
    # if averaged bad data (low intensity) is included in the 10 minute average.
    # NOTE: This may take care of many sampling issues as function of height
    # but also reduce sample size at higher heights.
...
def main():
    # Load the CDF file
    fdir = '/thumper/metdata/doppler-lidar/SGP/C1/vert/'
    kazrdir = '/thumper/metdata/radar/KAZR_SGP/'
    pblhdir = '/thumper/metdata/PBLH/SGP/'
    # sondedir = '/thumper/metdata/sonde/CACTI/M1/'
    winddir = '/thumper/metdata/doppler-lidar/SGP/C1/wind/'
    statdir = '/thumper/metdata/doppler-lidar/SGP/C1/stats/'
    ...
    # Get all the dates in the KAZR data.
    # dates = np.unique([re.search(r'\.(\d{8})\.\d{6}\.nc', fname).group(1) for fname in kazrnames])
    dates = np.unique([re.search(r'\.(\d{8})\.\d{6}\.cdf', fname).group(1) for fname in names])

    # Calculate the 10-minute running mean Doppler velocity as function of height from lidar data.
    output_dir = '/thumper/metdata/doppler-lidar/SGP_withrunningmeans/C1/'
    ...
    for date_str in dates:
        ...
        try:
            # Testing on '20140327' because its resampled distribution is
            # weird like the entire dataset.
            ...
            # statfile = [i for i in statnames if date_str in i][0]
            ...
            concatenateday(dayfiles,kazrfile,pblhfile,None,windfiles,date_str,26)
...
#old = pd.read_csv('/thumper/users/scott.powell/code/research-code/BNF/lidar/COR/updraft_output/updrafts_20190217.csv')
#new = pd.read_csv('/thumper/users/scott.powell/code/research-code/BNF/lidar/COR/updraft_output_resample/updrafts_resample_20190217.csv')

#(round(old['Height'])==495.0).sum()
#len(new[(new['Height']==0.495)*(new['PBL Height']>=495)])
