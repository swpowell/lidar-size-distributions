...
locations = ['C1_resample','E13_resample','E32_resample','E37_resample','E39_resample','E41_resample']
fnames500 = []
for loc in locations:
    fnames500.append(sorted(glob(basedir+loc+"/*.csv")))
# fnames = sorted(glob(fdir+"*.csv"))
fnames500 = [i for j in fnames500 for i in j]

#2143 to 2286 are bad. 
...
mask = np.isclose(combined_df_resam['Height'].values[:, None], values_to_keep, rtol=1e-5, atol=1e-8).any(axis=1)
...
# cond = (~np.isnan(combined_df_reg['PBL Height']))*(combined_df_reg['Height']<=combined_df_reg['PBL Height'])*(combined_df_reg['PBL Height'] <= top)*(combined_df_reg['PBL Height'] >= bottom)
# cond = cond * (combined_df_reg['Invalid Adjacent'] == False) * (combined_df_reg['Chord Length'] >= 1000)
# condr = (~np.isnan(combined_df_resam['PBL Height']))*(combined_df_resam['Height']*1000<=combined_df_resam['PBL Height'])*(combined_df_resam['Height']>0.03)*(combined_df_resam['PBL Height']<=top)*(combined_df_resam['PBL Height'] >= bottom)
# condr = condr * (combined_df_resam['Invalid Adjacent'] == False) * (combined_df_resam['Chord Length'] >= 1000)

cond = combined_df_reg['Invalid Adjacent'] == False
condr = combined_df_resam['Invalid Adjacent'] == False
...
A1_median = combined_df_resam[condr].groupby('Height', group_keys=False).apply(lambda x: x.nlargest(10000, 'Chord Length')['Chord Length'].median())
A2_median = combined_df_reg[cond].groupby('Height', group_keys=False).apply(lambda x: x.nlargest(10000, 'Chord Length')['Chord Length'].median())


fig, ax = plt.subplots(1,1,figsize=(6,6))
ax.plot(A1.index*1000,A1_median,'r',label='Resampled at 795 m')
ax.plot(A2.iloc[1:].index,A2_median.iloc[1:],'b',label='By Height')
# ax.plot(A1.index*1000,A1.values-A2.values[1:],'k',label='Difference')
