...
pd.to_csv(combined_df_reg,'combined_df_reg.csv')
pd.to_csv(combined_df_resam,'combined_df_resam.csv')
...
# Percentile
A1 = combined_df_resam[condr].groupby('Height')['Chord Length'].quantile(.5)
A2 = combined_df_reg[cond].groupby('Height')['Chord Length'].quantile(.5)
...
ax.plot(A1.index*1000,A1_mean,'r',label='Resampled at 795 m')
ax.plot(A2.iloc[1:].index,A2_mean.iloc[1:],'b',label='By Height')
...
# # Plot median of largest X chords.
# fig, ax = plt.subplots(1,1,figsize=(6,6))
# ax.plot(A1_large.index*1000,A1_large,'r',label='Resampled at 795 m')
# ax.plot(A2_large.iloc[1:].index,A2_large.iloc[1:],'b',label='By Height')
# # ax.plot(A1.index*1000,A1.values-A2.values[1:],'k',label='Difference')
# h,l = ax.get_legend_handles_labels()
# ax.legend(h,l)